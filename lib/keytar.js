    ...
    return {
      ...
      callback: function(err, val) {
        if (err) {
          ...
          val === undefined ? maybeCallback(err) : maybeCallback(err, val)
        }
      }
    }
...
module.exports = {
  ...
  setPassword: function (service, account, password, callback) {
    checkRequired(service, 'Service')
    checkRequired(account, 'Account')
    ...
    keytar.setPassword(service, account, password, ret.callback)
    return ret.value
  },
