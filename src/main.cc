...
void GetPassword(const Nan::FunctionCallbackInfo<v8::Value>& info)
{
    ...
    if (success) {
        v8::Local<v8::String> val = Nan::New<v8::String>(password.data(), password.length()).ToLocalChecked();
...
{
    bool success = keytar::DeletePassword(
        *v8::String::Utf8Value(info[0]),
        *v8::String::Utf8Value(info[1])
    );
...
{
    std::string password;
    bool success = keytar::FindPassword(*v8::String::Utf8Value(info[0]), &password);
